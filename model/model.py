...
class Guide(db.Model):
    ...
    nid = db.Column(db.String(10), primary_key=True)  # Unique guide ID
    user_id = db.Column(db.String(36), db.ForeignKey("users.id", ondelete="CASCADE"), nullable=False, unique=True)  # Foreign key to users table
    first_name = db.Column(db.String(80), nullable=True)
    last_name = db.Column(db.String(80), nullable=True)
    bio = db.Column(db.Text(80))
...
class Tour(db.Model):
    ...
    user_id = db.Column(db.String(36), db.ForeignKey("users.id", ondelete="CASCADE"), nullable=True)  # Foreign key to users
    guide_nid = db.Column(db.String(36), db.ForeignKey("guides.nid", ondelete="CASCADE"), nullable=True)  # Foreign key to guides
...
class Preset_Tour(db.Model):
    ...
    id = db.Column(db.Integer, db.ForeignKey("tours.id", ondelete="CASCADE"), primary_key=True)  # Foreign key to tours table
...
class Availability(db.Model):
    ...
    guide_id = db.Column(db.String(36), db.ForeignKey("guides.nid", name="fk_availability_guide", ondelete="CASCADE"), nullable=False)  # Foreign key to guides
    tour_id = db.Column(db.Integer, db.ForeignKey("preset_tours.id", name="fk_availability_tour", ondelete="CASCADE"), nullable=True)  # Foreign key to preset_tours
    start_date = db.Column(db.Date, nullable=False)
    end_date = db.Column(db.Date, nullable=False)
...
class Review(db.Model):
    ...
    user_id = db.Column(db.String(36), db.ForeignKey("users.id", ondelete="CASCADE"), nullable=False)  # Foreign key to users
    guide_id = db.Column(db.String(36), db.ForeignKey("guides.nid", ondelete="CASCADE"), nullable=False)  # Foreign key to guides
